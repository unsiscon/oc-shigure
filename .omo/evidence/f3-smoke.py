#!/usr/bin/env python3
# opco-shigure F3 REAL MANUAL QA driver (independent rerun of todo 8 smoke).
# Single consistent stack: python pty; same steps S1-S17 as t8; outputs f3-smoke.typescript.
...
# Evidence: .omo/evidence/f3-smoke.typescript (raw stream + screen snapshots).
...
OUTFILE = os.path.join(REPO, ".omo/evidence/f3-smoke.typescript")
...
def main():
    ...
    seen = []
    send(b"write the file .omo/evidence/f3-hello.txt with content hello\r", "S2 task prompt")
    t0 = time.time()
    done = False
    ...
    try:
        os.remove(os.path.join(REPO, ".omo/evidence/f3-hello.txt"))
        log("cleanup: removed .omo/evidence/f3-hello.txt")
    except OSError:
